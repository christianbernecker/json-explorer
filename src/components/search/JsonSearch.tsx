...
const JsonSearch: React.FC<JsonSearchProps> = ({
  ...
  useEffect(() => {
    console.log("JsonSearch - isVisible changed to:", isVisible);
    if (isVisible) {
      document.body.style.setProperty('--search-debug', '"SEARCH VISIBLE"');
    } else {
      document.body.style.setProperty('--search-debug', '"SEARCH HIDDEN"');
    }
  }, [isVisible]);

  return (
    <>
      {/* Debug-Element zur Anzeige des Zustands */}
      <div 
        style={{
          position: 'fixed',
          top: '40px',
      ...
      <div
        ...
        style={{ border: isVisible ? '3px solid red' : 'none' }}
      >
