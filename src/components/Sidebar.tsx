...
const Sidebar: React.FC<SidebarProps> = ({ isDarkMode }) => {
  ...
  const activeItemBg = isDarkMode ? 'bg-gray-700' : 'bg-white';
  const hoverBg = isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-white';
  const borderColor = isDarkMode ? 'border-gray-700' : 'border-gray-200';

  return (
    <div className={`fixed left-0 h-screen w-20 ${bgColor} border-r ${borderColor} flex flex-col items-center py-6 z-10`}>
      {/* Logo */}
      <div className="mb-8">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              to={item.path}
              className={`flex flex-col items-center justify-center w-16 h-16 rounded-lg transition-colors duration-200 ${textColor} ${hoverBg} 
                ${isActive ? activeItemBg + ' shadow-md font-medium' : ''}`}
            >
